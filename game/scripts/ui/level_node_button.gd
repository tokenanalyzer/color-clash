...
const _TOP_PADDING := 30.0 # room for the star row above the circle
...
func configure(id: int, p_state: StringName, p_stars: int) -> void:
	...
	custom_minimum_size = Vector2(NODE_RADIUS * 2.0 + 16.0, NODE_RADIUS * 2.0 + _TOP_PADDING + 10.0)
...
func _draw() -> void:
	...
	draw_circle(center + Vector2(0, 4), NODE_RADIUS, Color(0, 0, 0, 0.35))
	...
	draw_arc(center, NODE_RADIUS - 2.0, 0.0, TAU, 32, ring_color, 4.0, true)
	draw_circle(center + Vector2(-NODE_RADIUS * 0.28, -NODE_RADIUS * 0.32), NODE_RADIUS * 0.3, Color(1, 1, 1, 0.22))

	var font := ThemeDB.fallback_font
	if state == &"locked":
		draw_arc(center + Vector2(0, -7), 11.0, PI, TAU, 14, Color(0.85, 0.85, 0.92), 3.5, true)
		draw_rect(Rect2(center + Vector2(-12, -5), Vector2(24, 18)), Color(0.85, 0.85, 0.92))
	else:
	...
	if state == &"completed":
		for i in 3:
			var star_center := center + Vector2(float(i - 1) * 20.0, -NODE_RADIUS - 14.0)
			_draw_star(star_center, 7.5, Color(1, 0.85, 0.2) if i < stars else Color(0.32, 0.33, 0.4))
